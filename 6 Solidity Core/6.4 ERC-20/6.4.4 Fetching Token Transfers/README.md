...
Let's get all the dai transferred from the account we padded in stage two, between the `fromBlock` and the `toBlock` using the `eth_getLogs` method through the Alchemy SDK!
